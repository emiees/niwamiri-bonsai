...
| F019 | Ordenar fotos por fecha y hora, y mostrar la hora en la Galería | M5 Galería | P3 | `abierto` | 2026-04-04 | — | Las fotos del mismo día quedan mezcladas porque `takenAt` solo guarda fecha sin hora. Extraer hora del EXIF (ya disponible en el patrón `HH:MM:SS`), actualizar el date picker a datetime, y ordenar por timestamp completo. Mostrar hora junto a la fecha en el lightbox y en la agrupación de timeline. |
| F020 | Renderizar markdown en el Asistente IA | M8 AI / M14 AI General | P2 | `abierto` | 2026-04-04 | — | Los mensajes de los modelos se muestran como texto plano con `**asteriscos**` y `#` visibles. Agregar un renderer de markdown ligero (react-markdown o similar) en todos los componentes de chat de la app. |
