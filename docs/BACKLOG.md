...
| B013 | Fecha por defecto en nuevo recordatorio no usa el día seleccionado en el calendario | M9 Calendario | P2 | `en revisión` | 2026-03-29 | — | Al presionar el FAB (+) con un día seleccionado en el grid, `AddReminderSheet` siempre precargaba la fecha de hoy. Fix: se agrega prop `initialDate` al sheet y se le pasa `selectedDay` cuando hay uno seleccionado. |
| B014 | Recordatorio guardado se visualiza un día antes del seleccionado | M9 Calendario | P2 | `en revisión` | 2026-03-29 | — | `new Date('YYYY-MM-DD')` interpreta la fecha como UTC medianoche; en Argentina (UTC-3) eso equivale al día anterior a las 21:00. Fix: `new Date(date + 'T12:00:00')` para anclar a mediodía en zona local. |
| B015 | Eventos de cuidados (followup-reminder) no son editables desde el calendario | M9 Calendario | P3 | `en revisión` | 2026-03-29 | — | El botón de editar solo aparecía para `manual-reminder`. Se amplió la condición a `manual-reminder || followup-reminder`. El botón de eliminar ya funcionaba para todos los tipos. |
| B016 | Botón de cerrar lightbox de Galería queda detrás del reloj en iPhone 15 | M5 Galería | P2 | `en revisión` | 2026-04-04 | — | El header del lightbox ahora usa `paddingTop: calc(0.75rem + env(safe-area-inset-top))` para respetar el Dynamic Island. |
| B017 | Fecha al crear un cuidado se registra un día antes del seleccionado | M4 Cuidado | P1 | `en revisión` | 2026-04-04 | — | Misma causa raíz que B014. Fix aplicado en CareForm.tsx (6 ocurrencias) y ClassNotes.tsx (1 ocurrencia): `new Date(date)` → `new Date(date + 'T12:00:00')`. Journal.tsx usaba `T00:00:00` (local time) y no requirió cambios. |
| B018 | Sheet de nuevo evento en Calendario queda detrás del BottomNav | M9 Calendario | P1 | `en revisión` | 2026-04-04 | — | Misma causa raíz que B001. Fix aplicado en Calendar.tsx, BonsaiDetail.tsx (2 sheets), ClassNotes.tsx (2 sheets) y Backup.tsx: overlay z-40→z-[55], sheet z-50→z-[60]. |
...
| F008 | Ocultar "Sugerencia IA" (violeta) del filtro y leyenda del Calendario | M9 Calendario | P3 | `en revisión` | 2026-03-29 | — | El tipo `ai-suggestion` estaba en la UI (filtros, leyenda) pero ningún flujo de la app lo crea. Se removió del chip de filtro y de la leyenda de colores para evitar confusión. El tipo sigue definido en el schema para implementación futura. |
| F009 | Estilo con nombre informal en pantalla de edición | M3 Detalle | P3 | `en revisión` | 2026-03-29 | — | En la creación los chips de estilo mostraban nombre japonés + nombre informal en italic. En la edición solo se mostraba el japonés. Se modificó `chipRow` para aceptar `React.ReactNode` y se unificó el renderizado. |
| F010 | Selector de tamaño en pantalla de creación de árbol | M2 Inventario | P3 | `en revisión` | 2026-03-29 | — | El campo `size` no estaba expuesto en `AddBonsaiSheet`. Se agregó selector con chips igual al de estilo. |
| F011 | Rangos en cm en chips de tamaño (creación y edición) | M2 Inventario / M3 Detalle | P3 | `en revisión` | 2026-03-29 | — | Se agregó referencia visual de cm debajo del nombre en cada chip (ej: "Shohin / 15–25 cm"). Aplica en creación y edición. |
| F012 | Agregar tamaño Shito al catálogo | M2 Inventario / M3 Detalle | P3 | `en revisión` | 2026-03-29 | — | Faltaba el tamaño más pequeño (Shito, < 5 cm). Se agregó a `BonsaiSize` en schema.ts (cambio de tipo TypeScript, sin migración Dexie) y a los i18n es/en. |
| F013 | Subir múltiples fotos simultáneas desde la Galería | M5 Galería | P2 | `en revisión` | 2026-04-04 | — | `input[type=file]` ahora tiene `multiple`. Se genera una cola de fotos pendientes; el modal muestra de a una con indicador "N restantes" y botón "Omitir" para saltear. |
| F014 | Leer fecha EXIF de la foto al subirla y usarla como fecha del historial | M5 Galería | P3 | `en revisión` | 2026-04-04 | — | `extractExifDate()` escanea los primeros 64 KB del archivo buscando el patrón ASCII "YYYY:MM:DD HH:MM:SS" del EXIF. Si lo encuentra, pre-rellena el date picker; sino, usa fecha actual. |
| F015 | Editar la fecha de una foto guardada en la Galería | M5 Galería | P3 | `en revisión` | 2026-04-04 | — | En el lightbox, la fecha mostrada es un botón con ícono lápiz. Al tocarla aparece un date input inline con Guardar/Cancelar que llama a `updatePhoto({ takenAt })`. |
| F016 | Navegación entre fotos con flechas en el lightbox de la Galería | M5 Galería | P3 | `en revisión` | 2026-04-04 | — | Flechas ChevronLeft/ChevronRight sobre la imagen (suprimidas en el primer/último ítem). Indicador "N / Total" debajo de la imagen. |
| F017 | Backups automáticos locales al abrir la app (+ posible sync con Drive) | M11 Backup | P3 | `en revisión` | 2026-04-04 | — | Al abrir la app (WelcomeScreen, una vez por sesión), se genera un ZIP completo silenciosamente y se guarda en OPFS (`niwamiri-auto-backup.zip`). En Settings → Backup nueva tarjeta "Backup automático" muestra fecha/hora del último backup y botones "Generar ahora" y "Descargar". `lastAutoBackupAt` agregado a `AppConfig`. Sync con Drive queda para fase 2. |
| F018 | Acceder al detalle de un cuidado desde su evento en el Calendario | M9 Calendario / M4 Cuidado | P2 | `en revisión` | 2026-04-04 | — | Se agregó `careId?` a `CalendarEvent` (schema.ts, sin bump de versión Dexie). CareForm guarda el careId al crear el evento de cuidado. En EventRow, el título de eventos `care` con careId se muestra como enlace y navega a `/bonsai/:id/care/:careId`. El botón "Volver" ya funciona vía `navigate(-1)` del Header. |
...
| M014 | Incluir contexto de últimos cuidados en el Asistente IA por árbol | M8 Asistente IA | P3 | `en revisión` | 2026-04-04 | — | Los 3 providers (Gemini, Claude, OpenAI) ahora incluyen en el system prompt los últimos 7 cuidados con fecha, tipo, condición del árbol y descripción. Antes solo mostraban el tipo. |
