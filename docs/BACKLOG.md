...
| F013 | Subir múltiples fotos simultáneas desde la Galería | M5 Galería | P2 | `abierto` | 2026-04-04 | — | El input de carga de fotos solo permite seleccionar de a una. Habilitar `multiple` en el input y procesar las fotos en paralelo/secuencia mostrando progreso. |
| F014 | Leer fecha EXIF de la foto al subirla y usarla como fecha del historial | M5 Galería | P3 | `abierto` | 2026-04-04 | — | Al subir una foto, extraer la fecha de captura del metadata EXIF (si existe) y usarla como fecha de la foto en lugar de la fecha actual. Si no hay EXIF, usar la fecha actual como fallback. |
| F015 | Editar la fecha de una foto guardada en la Galería | M5 Galería | P3 | `abierto` | 2026-04-04 | — | En el lightbox solo se puede eliminar la foto. Agregar la posibilidad de editar su fecha (campo `takenAt` o `createdAt`), además de la descripción (F007). |
| F016 | Navegación entre fotos con flechas en el lightbox de la Galería | M5 Galería | P3 | `abierto` | 2026-04-04 | — | Al ver una foto en grande, mostrar flechas izquierda/derecha para pasar a la foto anterior/siguiente dentro del árbol, sin necesidad de cerrar y reabrir el lightbox. |
| F017 | Backups automáticos locales al abrir la app (+ posible sync con Drive) | M11 Backup | P3 | `abierto` | 2026-04-04 | — | Al iniciar la app, generar automáticamente un backup ZIP en el dispositivo (File System Access API o descarga directa). Fase 2: integración opcional con Google Drive similar al modelo de WhatsApp. Mostrar fecha y estado del último backup automático en Settings. |
...
| M014 | Incluir contexto de últimos cuidados en el Asistente IA por árbol | M8 Asistente IA | P3 | `abierto` | 2026-04-04 | — | Al armar el contexto del chat, incluir los N cuidados más recientes del árbol (tipo, fecha, condición, descripción). Permite al asistente dar sugerencias más precisas basadas en el historial real de cuidados. |
